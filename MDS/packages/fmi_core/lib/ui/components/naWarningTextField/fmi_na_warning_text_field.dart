...
class FmiNaWarningTextFieldWidget extends StatefulWidget {
  const FmiNaWarningTextFieldWidget({
    Key? key,
    this.controller,
    this.initValue,
    this.focusNode,
  ...
  final FmiNaWarningTextFieldModel? initValue;
  ...
  final Function(FmiNaWarningTextFieldModel?)? onChanged;
  ...
  final bool showNotAvailableButton;

  @override
  State<FmiNaWarningTextFieldWidget> createState() =>
      _FmiNaWarningTextFieldWidgetState();
}

class _FmiNaWarningTextFieldWidgetState
    extends State<FmiNaWarningTextFieldWidget> {
  final TextEditingController _textEditingController = TextEditingController();
  String? _warningText;
  bool _naSelected = false;
  bool _isEnabled = false;

  @override
  void initState() {
    if (widget.initValue != null) {
      _textEditingController.text = widget.initValue!.value ?? "";
      _naSelected = _textEditingController.text != "" ? false : true;
    }
  ...
  InputDecoration? rebuildInputDecoration() {
    ...
    if (_warningText != null) {
      result = widget.decoration?.copyWith(
        ...
        floatingLabelStyle: widget.decoration?.floatingLabelWarningStyle ??
            TextStyle(
  ...
  void onChangedState(String value) {
    ...
    if (widget.onChanged != null) {
      if (_naSelected && widget.showNotAvailableButton) {
        widget.onChanged!(FmiNaWarningTextFieldModel(value: null));
      } else if ((!_naSelected &&
              widget.showNotAvailableButton &&
              value.isNotEmpty) ||
          (!widget.showNotAvailableButton && value.isNotEmpty)) {
        widget.onChanged!(FmiNaWarningTextFieldModel(value: value));
      } else {
  ...
  void buttonOnPressed() {
    setState(() {
      ...
      _naSelected = !_naSelected;
      _isEnabled = !_isEnabled;
      _warningText = null;
      onChangedState(_textEditingController.text);
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    _isEnabled = widget.enabled ? !_naSelected : widget.enabled;
    ...
    return Row(
      children: [
        ...
        if (widget.showNotAvailableButton)
          Padding(
            padding: const EdgeInsets.only(left: FMIThemeBase.baseGapMedium),
            child: FmiToggleButton(
              onPressed: () => buttonOnPressed(),
              icon: FontAwesomeIcons.solidBan,
              enabled: widget.enabled,
              isToggled: _naSelected,
            ),
          ),
      ],
    );
